...
class ToothData(object):
    ...
    def from_treatment_item_metadata(self, t_i_meta):
        '''
        this input has come from a treatment_item
        (which is a class generated by a dialog which adds necessary data to
        a procedure code)
        '''
        self.proc_code = t_i_meta.parent_item.code
        if t_i_meta.is_fill:
            self.set_type(self.FILLING)
            self.set_surfaces(t_i_meta.surfaces)
            self.set_material(t_i_meta.material)

        elif t_i_meta.is_crown:
            self.set_type(self.CROWN)
            self.set_crown_type(self.proc_code.crown_type)

        elif t_i_meta.is_splint:
            self.set_type(self.ROOT)
            self.set_root_type("SPLINT")

        elif t_i_meta.is_root:
            self.set_type(self.ROOT)
